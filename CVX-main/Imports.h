...
bool ComputeHash(BYTE* data, DWORD length, BYTE* hashOutput)
{
    ...
    if (!CryptCreateHash(hProv, CALG_SHA_256, Encrypt(0), Encrypt(0), &hHash))
    {
        CryptReleaseContext(hProv, Encrypt(0));
        return false;
    }

    if (!CryptHashData(hHash, data, length, Encrypt(0)))
    {
    ...
    DWORD hashLen = Encrypt(32);
    if (!CryptGetHashParam(hHash, HP_HASHVAL, hashOutput, &hashLen, Encrypt(0)))
    {
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, Encrypt(0));
        return false;
...
public:
    ...
    {
        Hide_Ptr();
        MY_PEB* peb = (MY_PEB*)__readgsqword(Encrypt(0x60));
    ...
    void compute_hash(const BYTE* data, DWORD length, BYTE* hashOutput)
    {
        Hide_Ptr();
        HCRYPTPROV hProv = NULL;
        HCRYPTHASH hHash = NULL;
        CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT);
        CryptCreateHash(hProv, CALG_SHA_256, Encrypt(0), Encrypt(0), &hHash);
        CryptHashData(hHash, data, length, Encrypt(0));
        DWORD hashLen = Encrypt(32);
        CryptGetHashParam(hHash, HP_HASHVAL, hashOutput, &hashLen, Encrypt(0));
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, Encrypt(0));
        Hide_Ptr_End();
    }

    void get_api_hash(const char* moduleName, BYTE* savedHash)
    {
        Hide_Ptr();
        HMODULE hModule = get_module_handleA(moduleName); if (!hModule) { exit_application(); } // possible hook
        PIMAGE_DOS_HEADER pDosHeader = (PIMAGE_DOS_HEADER)hModule; if (!pDosHeader) { exit_application(); } // possible hook
        PIMAGE_NT_HEADERS pNtHeaders = (PIMAGE_NT_HEADERS)((BYTE*)pDosHeader + pDosHeader->e_lfanew);
        PIMAGE_SECTION_HEADER pSectionHeader = IMAGE_FIRST_SECTION(pNtHeaders);
        for (int i = 0; i < pNtHeaders->FileHeader.NumberOfSections; ++i)
        {
            if (import_strcmp((char*)pSectionHeader[i].Name, Encrypt(".text")) == Encrypt(0))
            {
                DWORD textSize = pSectionHeader[i].SizeOfRawData;
                BYTE* textData = (BYTE*)hModule + pSectionHeader[i].PointerToRawData;
                compute_hash(textData, textSize, savedHash);
                return;
            }
        }
        Hide_Ptr_End();
    ...
    bool ComputeDiskHash(const std::string& diskPath, BYTE* hashOut)
    {
        std::vector<BYTE> fileBuffer;
        if (!LoadFileToMemory(diskPath, fileBuffer))
            return false;

        PIMAGE_NT_HEADERS nt = GetNtHeaders(fileBuffer.data());
        if (!nt) return false;

        PIMAGE_SECTION_HEADER firstSection = (PIMAGE_SECTION_HEADER)((BYTE*)&nt->OptionalHeader + nt->FileHeader.SizeOfOptionalHeader);
        for (int i = Encrypt(0); i < nt->FileHeader.NumberOfSections; i++)
        {
            PIMAGE_SECTION_HEADER sec = &firstSection[i];
            if (import_strcmp((char*)sec->Name, Encrypt(".text")) == Encrypt(0))
            {
                BYTE* textData = fileBuffer.data() + sec->PointerToRawData;
                DWORD textSize = sec->SizeOfRawData;
                return ComputeHash(textData, textSize, hashOut);
            }
        }
        return false;
    }

    bool ComputeInMemoryHash(HMODULE hModule, BYTE* hashOut)
    {
        PIMAGE_DOS_HEADER dos = (PIMAGE_DOS_HEADER)hModule;
        PIMAGE_NT_HEADERS nt = GetNtHeaders((BYTE*)hModule);
        if (!nt) return false;
        ...
        for (int i = Encrypt(0); i < nt->FileHeader.NumberOfSections; i++)
        {
            ...
            if (import_strcmp((char*)sec->Name, Encrypt(".text")) == Encrypt(0))
            {
                BYTE* textData = (BYTE*)hModule + sec->VirtualAddress;
                DWORD textSize = sec->SizeOfRawData;
                return ComputeHash(textData, textSize, hashOut);
            }
        }
    ...
    bool VerifyModuleIntegrity(const std::string& moduleName)
    {
        HMODULE hModule = get_module_handleA(moduleName.c_str());
        if (!hModule) return false;

        BYTE diskHash[32] = { Encrypt(0) };
        BYTE memHash[32] = { Encrypt(0) };

        char systemPath[MAX_PATH] = { Encrypt(0) };
        GetSystemDirectoryA(systemPath, MAX_PATH);
        std::string fullPath = std::string(systemPath) + Encrypt("\\") + moduleName;

        if (!ComputeDiskHash(fullPath, diskHash))
            return false;

        if (!ComputeInMemoryHash(hModule, memHash))
            return false;

        return (import_memcmp(diskHash, memHash, Encrypt(32)) == Encrypt(0));
    }
    ...
    void hash_api_calls() // only call 1 time at the start of your program
    {
        ULONGLONG time_start = get_tick_count64();
        get_api_hash(Encrypt("kernel32.dll"), Kernel32);
        get_api_hash(Encrypt("kernelbase.dll"), Kernelbase);
        get_api_hash(Encrypt("ntdll.dll"), Ntdll);
        get_api_hash(Encrypt("msvcrt.dll"), Msvcrt);
        get_api_hash(Encrypt("vcruntime140.dll"), Vcruntime140);
        get_api_hash(Encrypt("ucrtbase.dll"), Ucrtbase);
        ULONGLONG Tick64 = get_tick_count64();
        if (!is_hash_valid(Kernel32, Encrypt(32)) || !is_hash_valid(Ntdll, Encrypt(32)) || !is_hash_valid(Msvcrt, Encrypt(32)) || !is_hash_valid(Kernelbase, Encrypt(32)) || !is_hash_valid(Vcruntime140, Encrypt(32)) || !is_hash_valid(Ucrtbase, Encrypt(32)))  // possible hook
        {
            exit_application();
        }
        hashed = true;
    ...
    void check_api_calls_static()
    {
        std::vector<void*> addys
        {
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("GetModuleHandleA")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("GetModuleHandleA")),
            (void*)get_function<void*>(Encrypt("user32.dll"), Encrypt("FindWindowA")),
            (void*)get_function<void*>(Encrypt("Advapi32.dll"), Encrypt("RegOpenKeyA")),
            (void*)get_function<void*>(Encrypt("Advapi32.dll"), Encrypt("RegQueryValueExA")),
            (void*)get_function<void*>(Encrypt("ntdll.dll"), Encrypt("NtSetInformationThread")),
            (void*)get_function<void*>(Encrypt("ntdll.dll"), Encrypt("NtQueryVirtualMemory")),
            (void*)get_function<void*>(Encrypt("ntdll.dll"), Encrypt("NtQuerySystemInformation")),
            (void*)get_function<void*>(Encrypt("ws2_32.dll"), Encrypt("recv")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("GetVolumeInformationA")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("GetVolumeInformationA")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("TerminateProcess")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("TerminateProcess")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("CreateThread")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("CreateThread")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("IsDebuggerPresent")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("IsDebuggerPresent")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("CheckRemoteDebuggerPresent")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("CheckRemoteDebuggerPresent")),
            (void*)get_function<void*>(Encrypt("kernel32.dll"), Encrypt("GetCurrentProcess")),
            (void*)get_function<void*>(Encrypt("kernelbase.dll"), Encrypt("GetCurrentProcess")),
            (void*)get_function<void*>(Encrypt("msvcrt.dll"), Encrypt("memcmp")),
            (void*)get_function<void*>(Encrypt("ucrtbase.dll"), Encrypt("memcmp"))
        };
        ...
        for (auto address : addys)
        {
            if (address)
            {
                while (*(BYTE*)(address) == Encrypt(0x90))
                {
                    exit_application();
                }
            }
        ...
        {
            Encrypt("kernel32.dll"),
            Encrypt("kernelbase.dll"),
            Encrypt("msvcrt.dll"),
            Encrypt("vcruntime140.dll"),
            Encrypt("ucrtbase.dll")
        };

        for (auto& mod : criticalModules)
        {
            if (!VerifyModuleIntegrity(mod))
            {
                exit_application();
            }
        }

        if (IsFunctionHooked(Encrypt("ntdll.dll"), Encrypt("NtQueryInformationProcess")))
        {
            exit_application();
        }

        if (IsFunctionHooked(Encrypt("ntdll.dll"), Encrypt("NtSetInformationThread")))
        {
            exit_application();
        }
    }

    __forceinline void check_return_values()
    {
        BOOLEAN bEnabled; FILETIME ft; SYSTEMTIME st; MEMORY_BASIC_INFORMATION mbi;         char envBuffer[256];
        auto lpFuncAddress = get_function<pdef_RtlAdjustPrivilege>(Encrypt("ntdll.dll"), Encrypt("RtlAdjustPrivilege"));
        DWORD ProcessId = get_function<DWORD(*)()>(Encrypt("kernel32.dll"), Encrypt("GetCurrentProcessId"))();
        DWORD ThreadId = get_function<DWORD(*)()>(Encrypt("kernel32.dll"), Encrypt("GetCurrentThreadId"))();
        HANDLE hProcess = get_function<HANDLE(*)()>(Encrypt("kernel32.dll"), Encrypt("GetCurrentProcess"))();
        HANDLE hThread = get_function<HANDLE(*)()>(Encrypt("kernel32.dll"), Encrypt("GetCurrentThread"))();
        DWORD Tick32 = get_function<DWORD(*)()>(Encrypt("kernel32.dll"), Encrypt("GetTickCount"))();
        ULONGLONG Tick64 = get_function<ULONGLONG(*)()>(Encrypt("kernel32.dll"), Encrypt("GetTickCount64"))();
        get_function<void(*)(LPFILETIME)>(Encrypt("kernel32.dll"), Encrypt("GetSystemTimeAsFileTime"))(&ft);
        get_function<void(*)(LPSYSTEMTIME)>(Encrypt("kernel32.dll"), Encrypt("GetSystemTime"))(&st);
        LPVOID pAlloc = get_function<LPVOID(*)(LPVOID, SIZE_T, DWORD, DWORD)>(Encrypt("kernel32.dll"), Encrypt("VirtualAlloc"))(nullptr, Encrypt(0x1000), MEM_COMMIT, PAGE_READWRITE);
        SIZE_T memQuery = get_function<SIZE_T(*)(LPCVOID, PMEMORY_BASIC_INFORMATION, SIZE_T)>(Encrypt("kernel32.dll"), Encrypt("VirtualQuery"))(pAlloc, &mbi, sizeof(mbi));
        DWORD envSize = get_function<DWORD(*)(LPCSTR, LPSTR, DWORD)>(Encrypt("kernel32.dll"), Encrypt("GetEnvironmentVariableA"))(Encrypt("PATH"), envBuffer, sizeof(envBuffer));
        HANDLE hStdOut = get_function<HANDLE(*)(DWORD)>(Encrypt("kernel32.dll"), Encrypt("GetStdHandle"))(STD_OUTPUT_HANDLE);
        NTSTATUS NtRet = lpFuncAddress(Encrypt(19), Encrypt(1), Encrypt(0), &bEnabled);
        if (ProcessId == Encrypt(0) || ThreadId == Encrypt(0) || hProcess == nullptr || hThread == nullptr || Tick32 <= Encrypt(0) || Tick64 <= Encrypt(0) || ft.dwLowDateTime == Encrypt(0) || ft.dwHighDateTime == Encrypt(0) || st.wYear == Encrypt(0) || st.wMonth == Encrypt(0) || st.wDay == Encrypt(0) || pAlloc == nullptr || memQuery == Encrypt(0) || envSize == Encrypt(0) || hStdOut == INVALID_HANDLE_VALUE || NtRet != Encrypt(0) || lpFuncAddress == nullptr)
        {
            exit_application();
        }
        get_function<BOOL(*)(LPVOID, SIZE_T, DWORD)>(Encrypt("kernel32.dll"), Encrypt("VirtualFree"))(pAlloc, Encrypt(0), MEM_RELEASE);
    }

    void check_api_calls()
    {
        if (hashed == Encrypt(1))
        {
            get_api_hash(Encrypt("kernel32.dll"), Kernel32_Check);
            get_api_hash(Encrypt("kernelbase.dll"), Kernelbase_Check);
            get_api_hash(Encrypt("ntdll.dll"), Ntdll_Check);
            get_api_hash(Encrypt("msvcrt.dll"), Msvcrt_Check);
            get_api_hash(Encrypt("vcruntime140.dll"), Vcruntime140_Check);
            get_api_hash(Encrypt("ucrtbase.dll"), Ucrtbase_Check);

            if (import_memcmp(Kernel32, Kernel32_Check, sizeof(Kernel32)) != false || import_memcmp(Kernelbase, Kernelbase_Check, sizeof(Kernelbase)) != false || import_memcmp(Ntdll, Ntdll_Check, sizeof(Ntdll)) != Encrypt(0) || import_memcmp(Msvcrt, Msvcrt_Check, sizeof(Msvcrt)) != Encrypt(0) || import_memcmp(Vcruntime140, Vcruntime140_Check, sizeof(Vcruntime140)) != Encrypt(0) || import_memcmp(Ucrtbase, Ucrtbase_Check, sizeof(Ucrtbase)) != Encrypt(0))
            {
                exit_application();
            }

            if (!is_hash_valid(Kernel32_Check, Encrypt(32)) || !is_hash_valid(Ntdll_Check, Encrypt(32)) || !is_hash_valid(Msvcrt_Check, Encrypt(32)) || !is_hash_valid(Kernelbase_Check, Encrypt(32)) || !is_hash_valid(Vcruntime140_Check, Encrypt(32)) || !is_hash_valid(Ucrtbase_Check, Encrypt(32)))
            {
                exit_application();
            }
            ...
            check_api_calls_static(); check_return_values();
        }
        else
        {
            check_api_calls_static(); check_return_values();
